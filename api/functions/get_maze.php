...
function get_maze($values) {
	...
	if(($result = get_maze_validate($inputs)) === false) {
		$continue = false;
	}
	...
	if($continue && ($questions = get_maze_build_list($result,$item_count)) === false) {
		$continue = false;
	}

	// Run SQL and proess results
	if($continue && ($xml = get_maze_build_xml($result,$questions,$item_count)) === false) {
		$continue = false;
	}
...
function get_maze_validate($inputs) {
...
function get_maze_build_list($options,$item_count = 5) {
...
/*
	$where = array();
	$where["core"][] = " maze_map.active = 't' ";
	if(!empty($options['generations'])) {
		$where["core"][] = " maze_map.generation_id in ('". implode("','",$options["generations"]) ."') ";
	}
	...
	if(empty($options['grade'])) {
		$where["core"][] = " maze_map.grade_id = '4' ";
	} else {
		$where["core"][] = " maze_map.grade_id = '". (is_array($options['grade']) ? $options['grade'][0] : $options['grade']) ."' ";
	}
...
function get_bundle_questions($id) {
    $q = "
		...
		from activities.maze as qq
		join activities.maze_map as qqm on
			qqm.maze_id = qq.id
--			and qqm.id in (select pk_id from activity_bundle_values where activity_bundle_id = '". $id ."')
		...
		join public.themes as t on
			t.id = qqm.theme_id
		left join activities.maze_answers as qqa on
			qqa.maze_id = qq.id
		order by
			qq.number
			,qqa.series
...
function get_question_bundle_media($id) {
	...
	$q = "
		...
		from activities.maze_media as qqm
     ...
     $output = array();
     while($row = db_fetch_row($res)) {
     	$output[$row["maze_id"]]["Media"][$row["media_group"]][$row["series"]] = array(
...
function get_maze_build_xml($options,$questions,$item_count) {
	$full_output = '';
	...
	foreach($questions as $row) {
		...
		if(!empty($row['FactId']) && !empty($options['addfacts'])) {
			$full_output .= maze_fact_xml($row);
		}

		$full_output .= maze_question_xml($row);
...
function maze_question_xml($row) {
