...
function get_tic_tac_know($values) {
	...
	audit_function("audits.tic_tac_know");
	...
	if(($result = get_tic_tac_know_validate($inputs)) === false) {
		$continue = false;
	}

	// Get item counts for the next portion
	$item_count = (!empty($_REQUEST["items"]) ? (int)$_REQUEST["items"] : 5);

	// Build the SQL needed for this api call
	if($continue && ($questions = get_tic_tac_know_build_list($result,$item_count)) === false) {
		$continue = false;
	}

	// Run SQL and proess results
	if($continue && ($xml = get_tic_tac_know_build_xml($result,$questions,$item_count)) === false) {
		$continue = false;
	}
...
function get_tic_tac_know_validate($inputs) {
...
function get_tic_tac_know_build_list($options,$item_count = 5) {
...
/*
	$where = array();
	$where["core"][] = " tic_tac_know_map.active = 't' ";
	if(!empty($options['generations'])) {
		$where["core"][] = " tic_tac_know_map.generation_id in ('". implode("','",$options["generations"]) ."') ";
	}
	...
	if(empty($options['grade'])) {
		$where["core"][] = " tic_tac_know_map.grade_id = '4' ";
	} else {
		$where["core"][] = " tic_tac_know_map.grade_id = '". (is_array($options['grade']) ? $options['grade'][0] : $options['grade']) ."' ";
	}
...
function get_bundle_questions($id) {
    $q = "
		...
		from activities.tic_tac_know as qq
		join activities.tic_tac_know_map as qqm on
			qqm.tic_tac_know_id = qq.id
--			and qqm.id in (select pk_id from activity_bundle_values where activity_bundle_id = '". $id ."')
		...
		join public.themes as t on
			t.id = qqm.theme_id
		left join activities.tic_tac_know_answers as qqa on
			qqa.tic_tac_know_id = qq.id
		order by
			qq.number
			,qqa.series
     ";
...
function get_question_bundle_media($id) {
	...
	$q = "
		...
		from activities.tic_tac_know_media as qqm
		join supplements.media_types as mt on
			mt.id = qqm.media_type_id
		where
			qqm.tic_tac_know_id in (
				select tic_tac_know_id
				from public.activity_bundle_values
				join activities.tic_tac_know_map as qqm on
				    qqm.id = pk_id
     ...
     $output = array();
     while($row = db_fetch_row($res)) {
     	$output[$row["tic_tac_know_id"]]["Media"][$row["media_group"]][$row["series"]] = array(
...
function get_tic_tac_know_build_xml($options,$questions,$item_count) {
	$full_output = '';
	...
	foreach($questions as $row) {
		...
		if(!empty($row['FactId']) && !empty($options['addfacts'])) {
			$full_output .= tic_tac_know_fact_xml($row);
		}

		$full_output .= tic_tac_know_question_xml($row);
...
function tic_tac_know_question_xml($row) {
...
function tic_tac_know_fact_xml($row) {
