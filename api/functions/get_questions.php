...
function get_bundle_questions($id) {
	...
	if(empty($_REQUEST['is_preview'])){
		...
		if(!empty($_REQUEST['id'])){
			$q .= "
				and qqm.quiz_question_id = '".$_REQUEST['id']."'
				";
		}
		if(!empty($_REQUEST['generation'])){
			$q .= "
				and qqm.generation_id = '".$_REQUEST['generation']."'
				";
		}
	}
	$q .= "
	...
	$q .= "
     ...
     while($row = db_fetch_row($res)) {
     	$should_add_row = true;
     	if(!empty($_REQUEST['is_preview'])){
     		if($_REQUEST['is_preview'] == true){
	     		if(!empty($_REQUEST['number'])){
	     			if($row["number"] != $_REQUEST['number']){
	     				$should_add_row = false;
	     			}
	     		}
	     		if(empty($row["activity_bundle_id"])){
	     			$row["activity_bundle_id"] = '0';
	     		}
	     		if(empty($row["activity_bundle_value_id"])){
	     			$row["activity_bundle_value_id"] = '0';
	     		}
	     	}
     	}
     	if($should_add_row == true){
