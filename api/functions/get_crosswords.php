...
function get_crosswords($values) {
	...
	audit_function("audits.crosswords");
	...
	$inputs['game'] = "crossword";

	// validate the inputs (worlds, themes, genres, etc)
	if(($result = get_crossword_validate($inputs)) === false) {
		$continue = false;
	}
	...
	if($continue && ($questions = get_crossword_build_list($result,$item_count)) === false) {
		$continue = false;
	}

	// Run SQL and proess results
	if($continue && ($xml = get_crossword_build_xml($result,$questions,$item_count)) === false) {
		$continue = false;
	}
...
function get_crossword_validate($inputs) {
...
function get_crossword_build_list($options,$item_count = 500) {

	$where = array();
	$where["core"][] = " cm.active = 't' ";
	if(!empty($options['generations'])) {
		$where["core"][] = " cm.generation_id in ('". implode("','",$options["generations"]) ."') ";
	}
	...
	if(empty($options['grade'])) {
		$where["core"][] = " cm.grade_id = '4' ";
	} else {
		$where["core"][] = " cm.grade_id = '". (is_array($options['grade']) ? $options['grade'][0] : $options['grade']) ."' ";
	}
 ...
 //    	select
	// 		'". $id['id'] ."'
	// 		,crossword_map.id
 //    	from activities.crossword_map
 //    	join public.worlds on
 //    		worlds.id = crossword_map.world_id
 //    		". (!empty($where["worlds"]) ? " and ( ". implode(' or ', $where["worlds"]) ." ) " : "") ."
 //    	join public.themes on
 //    		themes.id = crossword_map.theme_id
 ...
 //    db_query($q,"Creating random list");
    ...
    // $media = get_crossword_bundle_media($id["id"]);
...
function get_bundle_questions($id,$where,$item_count) {
    $q = "
   		select 
			...
			,cm.grade_id
			,cm.generation_id
			,json_agg(cmedia.filename order by series) AS media
		from activities.crosswords as c
		join activities.crossword_map as cm on
			cm.crossword_id = c.id
		...
		join public.themes as t on
			t.id = cm.theme_id
			". (!empty($where["themes"]) ? " and ( ". implode(' or ', $where["themes"]) ." ) " : "") ."
		left join activities.crossword_media as cmedia on
			cmedia.crossword_id = c.id
		where
			". (!empty($where["core"]) ? implode(" and ", $where["core"]) : '')."
		group by
			c.id
...
// function get_crossword_bundle_media($id) {
...
// 		from activities.crossword_media as cm
...
//      $output = array();
//      while($row = db_fetch_row($res)) {
//      	$output[$row["crossword_id"]]["Media"][$row["media_group"]][$row["series"]] = array(
...
function get_crossword_build_xml($options,$questions,$item_count) {
	$full_output = '';
	...
	foreach($questions as $row) {
		...
		$full_output .= crossword_question_xml($row);
...
function crossword_question_xml($row) {
	...
	// $output .= get_crossword_media_xml($row);
...
// function get_crossword_media_xml($row) {
