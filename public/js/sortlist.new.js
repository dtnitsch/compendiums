function sortlist(){
    ...
    remote.sort = function(obj,custom){
        ...
        if(remote.cname == remote.col_last) {
            if(!(remote.p && remote.p_change) && change_dir) {
...
// remote.setup_onclicks();
    ...
    function no_results(obj) {
        var tbody = document.getElementById(obj.id).getElementsByTagName("tbody")[0];
        var colspan = document.getElementById(obj.id).getElementsByTagName('th').length;

        tbody.innerHTML = '<tr><td colspan="'+ colspan +'" class="no-results">No Results Found</td></tr>';
    }
    ...
    function loading(obj) {
        ...
        tbody.innerHTML = '<tr><td colspan="'+ colspan +'" class="loading"><div><span>Loading...</span></div></td></tr>';
    }
