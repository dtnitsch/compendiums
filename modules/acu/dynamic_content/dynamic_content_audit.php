...
if(!has_access('dynamic_content_audit')) { back_redirect(); }
...
#   DB Queries
##################################################
$info = db_fetch("select * from public.dynamic_content where id='". $id ."'",'Getting Dynamic Content');
...
##################################################
?>
	<h2 class='dynamic-content'>Audit Dynamic Content: <?php echo $info["title"]; ?></h2>
  
  <div class='content_container'>

	<?= dynamic_content_navigation($id,"audit") ?>
	...
	<table id='asl_sort' cellpadding="0" cellspacing="0" class='asl_sort'>
		...
		<tr onclick='window.location="/acu/dynamic-content/edit/?id={{id}}"'>
			<td>{{column_name}}</td>
			<td>{{full_name}}</td>
			<td>{{old_value}}</td>
			<td>{{new_value}}</td>
...
<script type="text/javascript">
	...
	asl_sort.init('/ajax.php',{
		id:'asl_sort'
		,data: 'apid=5cfcb6280c27ab5981cfce764d3ec91d&id=<?php echo $id; ?>'
		,filters: 'filters'
		,type: "pagination"
	});
