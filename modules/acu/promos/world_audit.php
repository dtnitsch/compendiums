...
if(!has_access("world_audit")) { back_redirect(); }
...
##################################################
?>
	<h2 class='worlds'>Audit World: <?php echo $info["title"]; ?></h2>
  
  <div class='content_container'>

	<?= world_navigation($id,"audit") ?>
	...
	<table id='asl_sort' cellpadding="0" cellspacing="0" class='asl_sort'>
		...
		<tr onclick='window.location="/acu/worlds/edit/?id={{id}}"'>
			<td>{{column_name}}</td>
			<td>{{full_name}}</td>
			<td>{{old_value}}</td>
			<td>{{new_value}}</td>
