...
*/
	...
	if (!error_message()) {
		...
		if (!empty($old_answers_delete)) {
			...
			$q = "
				update activities.quiz_question_answers
				set
					active = 'f'
					,is_correct = 'f'
					,modified = '".$time."'
			...
			foreach ($new_answers_add as $k => $v) {
				...
				$q = "
					insert into activities.quiz_question_answers (
						...
						".db_prep_sql((int) $quiz_question_id)."
						,(select max(series) + 1 from activities.quiz_question_answers where quiz_question_id = ".db_prep_sql((int) $quiz_question_id).")
	...
	if (!error_message() && !empty($_POST["fact_id"])) {
		...
		$table_info["table_columns"][] = array(
			...
			,"hash" => db_prep_sql(md5(trim($_POST["fact"])))
			,"number" => db_prep_sql(trim($_POST["number"]))
		);
	...
	if (!error_message()) {
		...
		$q = "
			...
			where
				quiz_question_id = ".db_prep_sql((int) $_POST["id"])."
				and active
		";
		...
		foreach ($old_map_update as $k => $v) {
			...
			$q = "
				update activities.quiz_question_map
				set
					world_id = ".db_prep_sql((int) $v["world_id"])."
					,theme_id = ".db_prep_sql((int) $v["theme_id"])."
					,grade_id = ".db_prep_sql((int) $v["grade_id"])."
					,generation_id = ".db_prep_sql((int) $v["generation_id"])."
					,modified = '".$time."'
				where
					id = ".db_prep_sql((int) $k)."
			";
		...
		if (!empty($grid)) {
			...
			foreach ($grid as $k => $v) {
				...
				if (
					...
					empty($v["theme_id"]) &&
					empty($v["grade_id"]) &&
					empty($v["generation_id"])
				) { continue; }

				$q[] = "
					(
						".db_prep_sql((int) $quiz_question_id)."
						,".db_prep_sql((int) (!empty($v["world_id"]) ? $v["world_id"] : 0))."
						,".db_prep_sql((int) (!empty($v["theme_id"]) ? $v["theme_id"] : 0))."
						,".db_prep_sql((int) (!empty($v["grade_id"]) ? $v["grade_id"] : 0))."
						,".db_prep_sql((int) (!empty($v["generation_id"]) ? $v["generation_id"] : 0))."
					)
				";
	...
	// MEDIA UPDATES //
	///////////////////
	if (!update_quiz_question_media($_POST["id"], $_POST["media"])) { error_message("An error occured while updating media."); }

	if (!error_message()) {

		// audit('table_update',$table_info);

		$redirection_path = "/acu/quiz-questions/edit/?id=".$_POST["id"];
