...
if(!empty($_POST['filters'])) {
	foreach($_POST['filters'] as $k => $v) {
		...
		// Condensed username search
			...
			$v = str_replace(' ','%',$v);
			$where[] = " (sas.user_firstname ||' '|| sas.user_lastname) ilike '%". $v ."%' ";
	...
	$where[] = " sas.created >= '". $start_date ."' and sas.created <= '". $end_date ."' ";
...
$q = "
	...
	where
		 sas.created_date >= '2016-02-01' and sas.created_date <= '2016-04-10' 
	group by
		sas.student_firstname
		,sas.grade_id
		,sas.grade
