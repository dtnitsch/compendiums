...
$security_check_list = ['crosswords_list','crosswords_add','crosswords_edit','crosswords_delete'];
$security_list = has_access(implode(",",$security_check_list)); 
if(empty($security_list['crosswords_list'])) { back_redirect(); }
...
?>
<div class='clearfix'>
	<h2 class='crosswords'>Crossword List</h2>
...
?>
		<button onclick='window.location.href="/acu/crosswords/add/"' class='add'>Add New Questions</button>
	</div>
</div>
...
<?php
	echo build_db_select(get_worlds(),"cwm.world_id", "world");
?>
		</div>
		<div class='inputs float_left'>
			<label for='generation'><b>Generation</b></label><br>
<?php
	echo build_db_select(get_generations(),"cwm.generation_id", "generation");
?>
		</div>
		<div class='inputs float_left'>
			<label for='themes'><b>Themes</b></label><br>
<?php
	echo build_db_select(get_themes(),"cwm.theme_id", "theme");
?>
		</div>
		<div class='inputs float_left'>
			<label>&nbsp;</label><br>
	...
	<table id='asl_sort' cellpadding="0" cellspacing="0" class='asl_sort'>
		...
		<tr onclick='window.location="/acu/crosswords/edit/?id={{id}}"'>
			<td>{{word}}</td>
			<td>{{modified}}</td>
			<td rel='nolink' class='options'>
				<a href='/acu/crosswords/delete/?id={{id}}' title="Delete: {{crosswords}}" class='delete'></a>
			</td>
		</tr>
		</tbody>
	</table>
...
<script type="text/javascript">
    ...
    asl_sort.init('/ajax.php',{
        id:'asl_sort'
        ,data: 'apid=e48ab82b0c4a13ae17a469ad232f02fe'
