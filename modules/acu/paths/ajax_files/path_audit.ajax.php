...
if(!empty($_POST['filters'])) {
	foreach($_POST['filters'] as $k => $v) {
		$v = trim($v);
		if($v == '') { continue; }
		$where .= " and system_field_log.". $k ." ilike '%". $v ."%' ";
	}
}
...
$q = "
	select 
		system_field_log.id
		,system_field_log.table_log_id
		,system_field_log.column_name
		,system_field_log.old_value
		,system_field_log.new_value
		,(system.user.first || ' ' || system.user.last) as full_name
		,to_char(system_field_log.created,'Mon DD, YYYY HH:MI:SSam') as created
	from audits.system_table_log
	join audits.system_field_log on
		system_field_log.table_log_id = system_table_log.id
		and system_table_log.primary_key_id='". $_POST['id'] ."'
	left join system.user on system.user.id = system_table_log.user_id
	where
		system_table_log.schema_name = '". $vars['db_schema'] ."'
		and system_table_log.table_name = '". $vars['db_table'] ."'
		". $where ."
		". $order ."
		". $limit ."
";
