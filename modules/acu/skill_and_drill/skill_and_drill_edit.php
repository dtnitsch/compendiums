...
if(!has_access("skill_and_drill_edit")) { back_redirect(); }
...
if(empty($_POST)) {
	$q = "
		select
			...
			,sd.points
			,sd.description
			,json_agg(sda.is_correct order by sda.id) as is_correct_array
			,json_agg(sda.answer order by sda.id) as answer_array
			,json_agg(sda.id order by sda.id) as answer_id_array
	...
	$info = db_fetch($q,"Getting Skill and Drills");
	...
	$q = "
		...
		order by
			...
			,generation_id
	";
	$res = db_query($q,'Getting Skill and Drills Grid');
...
?>
	...
	<h2 class='skill-and-drill'>Edit Skill and Drills: <span class='small'><?php echo htmlspecialchars($info["question"]); ?></span></h2>

  <div class='content_container'>

	<?= skill_and_drill_navigation($id,"edit") ?>
