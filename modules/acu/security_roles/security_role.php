...
$security_check_list = ['security_role_list','security_role_add','security_role_edit','security_role_delete'];
$security_list = has_access(implode(",",$security_check_list)); 
if(empty($security_list['security_role_list'])) { back_redirect(); }
...
?>
<div class='clearfix'>
	<h2 class='security-roles'>Security Role List</h2>
  
  <div class='right float_right buttons'>
  <?php
    $add_button = "";
    if(!empty($security_list["security_role_add"])) {
    	$add_button = "<button onclick='window.location.href=\"/acu/security-roles/add/\"' class='add'>Add New Security Role</button>";
    }

    $edit_onclick = "";
    if(!empty($security_list['security_role_edit'])) {
        $edit_onclick = " onclick='window.location=\"/acu/security-roles/edit/?id={{id}}\"'";
    }

    $delete_link = "";
    if(!empty($security_list['security_role_delete'])) {
        $delete_link = '<a href="/acu/security-roles/delete/?id={{id}}" title="Delete: {{title}}" class="delete"></a>';
    }
?>
...
<?php echo dump_messages(); ?>
	...
	<form id="form_filters" method="" action="" onsubmit="return false;">
		...
		<div class='inputs float_left'>
			<label for='title'><b>Security Role</b></label><br>
	...
	<table id='asl_sort' cellpadding="0" cellspacing="0" class='asl_sort'>
		<thead>
		<tr>
			<th data-col="title">Security Role</th>
			<th data-col="alias">Alias</th>
			<th data-col="modified">Date Modified</th>
			<th nosort class='options' style='width: 1%;'><img src='/images/options.png' /></th>
		</tr>
		</thead>
...
<script type="text/javascript">
	...
	asl_sort.init('/ajax.php',{
		id:'asl_sort'
		,data: 'apid=a9068764f45c45d5799d85488d242fab'
