...
if(!empty($_POST) && !error_message()) {
	...
	if(!error_message()) {
		...
		$pub = db_fetch("select id from public.compendium where key = '". db_prep_sql($key) ."'","Getting id");

		$q = "
			update public.compendium set
			...
			where
				key = '". db_prep_sql($key) ."'
		";
		$res = db_query($q, "Updating Compendium"); 
		
		if(!error_message()) {
			$compendium_id = $pub['id'];
			...
			$q = "
				select
					compendium_list_map.*
					,list.key
				from public.compendium_list_map
				join public.list on
					list.id = compendium_list_map.list_id
				where compendium_list_map.compendium_id = '". $compendium_id ."'
				order by
					list_id
			";
			$res = db_query($q,"Getting compendium list map list ids");
			$existing_list_ids = [];
			$existing_list = [];
			while($row = db_fetch_row($res)) {
			...
			foreach($existing_list_ids as $k => $v) {
				...
				if(empty($remove[$k])) {
					...
					if(count($updates)) {
						...
						$q = "
							update public.compendium_list_map set
								". implode(', ',$updates) ."
								,modified = now()
							where
								id = '". $existing_list[$k]['id'] ."'
						";
						db_query($q,"Updating compendium list");
					}
				}
			}
			...
			if(!empty($remove)) {
				$q = "
					delete from public.compendium_list_map
					where
						compendium_list_map.compendium_id = '". $compendium_id ."'
						and list_id in (". implode(',',$remove) .")
				";
				db_query($q,"Removing Assets from compendium");				
			}
			...
			if(!empty($add)) {
				...
				$q = "insert into compendium_list_map (
						compendium_id
						,list_id
						,label
						,randomize
				...
				db_query($q,"Inserting Assets for compendium");
			}

			// $q = "insert into compendium_list_map (compendium_id,asset_id,created,modified) values ". implode(',',$map_ids);
			// db_query($q,"Inserting compendium asset map");
		

			$redirection_path = '/compendiums/edit/?key='. $key;
